...
// not applicable for the graph with the negative weight cycle
// dijkstra does not work on negative work cycle and with negative weights
class Solution
{
	public:
    ...
    vector <int> dijkstra(int V, vector<vector<int>> adj[], int S)
    {
        ...
        // from sourcre to source the distance is always zero
        dist[S]=0;
        pq.push({0,S});
        ...
        while(!pq.empty())
        {
            ...
            for(auto it:adj[node])
            {
                ...
                if(dis+egdeweight<dist[adjNode])
                {
                    dist[adjNode]=dis+egdeweight;
