...
//         return false;
//     }
// //  applying the concept of longest increasing  subsequence
...
class Solution {
public:
bool isplacable(vector<vector<int>>& cub,int curr,int prev){
if(cub[curr][0]<=cub[prev][0] &&
cub[curr][1]<=cub[prev][1] &&
cub[curr][2]<=cub[prev][2])return true;
return false;
}
int baba(vector<vector<int>>& nums,int n,int c,int p, vector<vector<int>>&dp){
    if(c<0)return 0;
 if(dp[c][p+1]!=-1)return dp[c][p+1];
    int pick=0;
    if(p==-1 || isplacable(nums,c,p))//if placable
    pick=nums[c][2]+baba(nums,n,c-1,c,dp);//add height and call for lower block
    int notpick=baba(nums,n,c-1,p,dp);//if not,just call 
    return dp[c][p+1]=max(pick,notpick);
}
    int maxHeight(vector<vector<int>>& cub) {int n=cub.size();
        for(int i=0;i<cub.size();i++){
            sort(cub[i].begin(),cub[i].end());//sort to get max height
        }
         vector<vector<int>>dp(n+1,vector<int>(n+1,-1));
        sort(cub.begin(),cub.end());//sort to get inceasing width
        return baba(cub,cub.size(),cub.size()-1,-1,dp);//call from n-1
    }
};
